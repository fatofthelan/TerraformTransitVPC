...
resource "aws_subnet" "transit_vpc_untrust_subnet_az1" {
  ...
  cidr_block        = join("", [var.transit_vpc_cidr_prefix, "1.0/24"])
...
resource "aws_subnet" "transit_vpc_untrust_subnet_az2" {
  vpc_id            = aws_vpc.transit_vpc.id
  cidr_block        = join("", [var.transit_vpc_cidr_prefix, "2.0/24"])
...
resource "aws_subnet" "transit_vpc_trust_subnet_az1" {
  vpc_id            = aws_vpc.transit_vpc.id
  cidr_block        = join("", [var.transit_vpc_cidr_prefix, "10.0/24"])
  availability_zone = data.aws_availability_zones.available.names[0]
...
resource "aws_subnet" "transit_vpc_trust_subnet_az2" {
  vpc_id            = aws_vpc.transit_vpc.id
  cidr_block        = join("", [var.transit_vpc_cidr_prefix, "20.0/24"])
  availability_zone = data.aws_availability_zones.available.names[1]
...
resource "aws_eip" "firewall_1_management_public_ip" {
  ...
  depends_on = [
    aws_vpc.transit_vpc,
    aws_internet_gateway.transit_internet_gateway,
    aws_network_interface.firewall_1_management_network_interface,
  ]
...
resource "aws_eip" "firewall_1_untrust_public_ip" {
  ...
  depends_on = [
    aws_vpc.transit_vpc,
    aws_internet_gateway.transit_internet_gateway,
    aws_network_interface.firewall_1_untrust_network_interface,
  ]
...
resource "aws_eip" "firewall_2_management_public_ip" {
  ...
  depends_on = [
    aws_vpc.transit_vpc,
    aws_internet_gateway.transit_internet_gateway,
    aws_network_interface.firewall_2_management_network_interface,
  ]
...
resource "aws_eip" "firewall_2_untrust_public_ip" {
  ...
  depends_on = [
    aws_vpc.transit_vpc,
    aws_internet_gateway.transit_internet_gateway,
    aws_network_interface.firewall_2_untrust_network_interface,
  ]
...
resource "aws_network_interface" "firewall_1_management_network_interface" {
  ...
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "1.10"])]
...
resource "aws_network_interface" "firewall_1_untrust_network_interface" {
  ...
  source_dest_check = false
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "1.20"])]
...
resource "aws_network_interface" "firewall_1_trust_network_interface" {
  ...
  source_dest_check = false
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "10.20"])]
...
resource "aws_network_interface" "firewall_2_management_network_interface" {
  ...
  source_dest_check = false
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "1.11"])]
...
resource "aws_network_interface" "firewall_2_untrust_network_interface" {
  ...
  source_dest_check = false
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "1.21"])]
...
resource "aws_network_interface" "firewall_2_trust_network_interface" {
  ...
  source_dest_check = false
  private_ips_count = 1
  private_ips       = [join("", [var.transit_vpc_cidr_prefix, "10.21"])]
